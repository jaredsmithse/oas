...
describe('TypeScript dist quirks', function () {
  /**
   ...
   */
  it('should be able to generate enums', function () {
    const spec = {
      openapi: '3.0.0',
      info: {
        title: 'fixed enums',
    ...
    expect(oas.operation('/anything', 'post').getParametersAsJsonSchema()[0].schema.properties.enumType).to.deep.equal({
      type: 'string',
      enum: ['pug', 'cat'],
    });
  });
